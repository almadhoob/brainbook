...
//gets all private & public posts that the user can see
func (db *DB) AllPosts(userID int) ([]Post, error) {
...
// Gets all limited posts visible to the given user
func (db *DB) LimitedPostsForUser(userID int) ([]Post, error) {
	ctx, cancel := context.WithTimeout(context.Background(), defaultTimeout)
	defer cancel()
